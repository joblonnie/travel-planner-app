...
export function BookingModal({ activity, dayId, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-md sm:p-4" onClick={onClose}>
      <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl max-w-md w-full max-h-[85vh] sm:max-h-[90vh] overflow-y-auto border border-spain-yellow/10" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-spain-yellow/10 bg-gradient-to-r from-warm-50 to-amber-50/30 rounded-t-3xl">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-gradient-to-br from-spain-yellow to-amber-400 rounded-xl flex items-center justify-center shadow-sm">
              <Ticket size={16} className="text-white" />
            </div>
            <h3 className="font-bold text-spain-dark">{t('booking.title')}</h3>
          </div>
          <button onClick={onClose} className="p-1.5 hover:bg-spain-red/10 rounded-xl transition-colors">
            <X size={18} className="text-gray-400" />
          </button>
        </div>

        <div className="p-4 space-y-4">
          <div className="bg-gradient-to-r from-spain-red/5 to-spain-yellow/5 rounded-xl p-3 border border-spain-yellow/10">
            <p className="font-bold text-sm text-spain-dark">{activity.nameKo}</p>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-spain-yellow/30 focus:border-spain-yellow/60 outline-none transition-all bg-gray-50/30 focus:bg-white"
            />
          </div>
          ...
          <div>
            ...
            <label className="flex items-center gap-2.5 px-4 py-3 border-2 border-dashed border-gray-200 rounded-xl text-sm text-gray-400 hover:border-spain-yellow hover:text-spain-yellow-dark cursor-pointer transition-all bg-gray-50/30 hover:bg-amber-50/30">
              <Upload size={16} />
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-spain-yellow/30 focus:border-spain-yellow/60 outline-none resize-none transition-all bg-gray-50/30 focus:bg-white"
            />
          </div>
        </div>
