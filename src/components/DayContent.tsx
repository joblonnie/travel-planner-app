...
import { Calendar, MapPin, Navigation, Plus, Clock, Compass, Hotel, Check, ChevronLeft, ChevronRight, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="group/insert flex items-center py-0.5">
      ...
      >
        ...
        <span className="hidden sm:inline font-medium tracking-wide">여기에 추가</span>
      </button>
      <div className="flex-1 h-px bg-gradient-to-r from-transparent via-gray-200/40 to-transparent group-hover/insert:via-spain-red/15 transition-colors" />
    </div>
  );
...
export function DayContent() {
  const { days, currentDayIndex, reorderActivities, getTotalCost, getDayCost, getDayActualCost, getTotalExpenses, getAllDestinations, goToNextDay, goToPrevDay, totalBudget } = useTripStore();
  ...
  return (
    <main className="flex-1 bg-gradient-to-br from-gray-50/80 via-white/40 to-gray-100/50">
      ...
      <div className="max-w-4xl mx-auto px-3 py-4 sm:px-4 md:p-6 space-y-4 sm:space-y-5">
        ...
        {currentDay.activities.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {currentDay.activities.map((act, i) => {
              ...
              const isSkip = act.isSkipped;
              return (
                <div
                  key={act.id}
                  className={`flex items-center gap-1.5 pl-1 pr-2.5 py-1 rounded-full text-[11px] font-semibold transition-all ${
                    isDone
                      ? 'bg-emerald-100 text-emerald-800 border border-emerald-300/70'
                      : isSkip
                      ? 'bg-amber-100 text-amber-700 border border-amber-300/70 line-through opacity-70'
                      : 'bg-white text-gray-800 border border-gray-300 shadow-[0_1px_3px_rgba(0,0,0,0.06)]'
                  }`}
                >
                  <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold flex-shrink-0 ${
                    isDone
                      ...
                      : 'bg-gradient-to-br from-spain-red to-spain-red-dark text-white shadow-sm'
                  }`}>
                    {isDone ? <Check size={9} /> : i + 1}
                  </span>
                  <span>{act.nameKo}</span>
                  <span className={`font-mono text-[10px] ${isDone ? 'text-emerald-600' : isSkip ? 'text-amber-500' : 'text-gray-500'}`}>{act.time}</span>
                </div>
              );
            })}
          </div>
        ...
        {currentDay.accommodation?.name && (
          <div className="bg-purple-50/40 backdrop-blur-xl rounded-2xl border border-purple-200/30 px-4 py-3.5 shadow-[0_2px_10px_rgba(139,92,246,0.06)]">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-[10px] font-bold text-purple-600 uppercase tracking-wider">숙소</span>
            </div>
            ...
            <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-400">
              {currentDay.accommodation.checkIn && <span className="bg-purple-100/60 px-2 py-0.5 rounded-md">체크인 {currentDay.accommodation.checkIn}</span>}
              {currentDay.accommodation.checkOut && <span className="bg-purple-100/60 px-2 py-0.5 rounded-md">체크아웃 {currentDay.accommodation.checkOut}</span>}
            </div>
        ...
        <div>
          <h2 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-2 tracking-tight">
            {t('day.todaySchedule')}
            <span className="text-[10px] bg-gray-100/80 text-gray-400 px-2.5 py-0.5 rounded-full font-medium">{t('day.dragToReorder')}</span>
          </h2>
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={currentDay.activities.map((a) => a.id)} strategy={verticalListSortingStrategy}>
              <div className="space-y-1">
                ...
                <InsertButton onClick={() => { setInsertAtIndex(0); setShowAdd(true); }} />
                  ...
                  <div key={activity.id}>
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <ActivityCard activity={activity} dayId={currentDay.id} index={index} totalCount={currentDay.activities.length} />
                      </div>
                    </div>
                    {/* Insert button between activities */}
                    <div className="pl-9">
                      <InsertButton onClick={() => { setInsertAtIndex(index + 1); setShowAdd(true); }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Bottom add button (compact fallback) */}
          <button
            onClick={() => { setInsertAtIndex(undefined); setShowAdd(true); }}
            className="w-full py-2.5 border border-dashed border-gray-200/60 rounded-xl text-gray-400 hover:border-spain-red/30 hover:text-spain-red transition-all flex items-center justify-center gap-1.5 mt-2 text-xs hover:bg-red-50/20"
          >
            <Plus size={14} />
            {t('day.addActivity')}
          </button>
