...
function App() {
  ...
  // Auth loading or checking session
  if (loading) {
    return (
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-warm-50 px-4">
        <div className="max-w-sm w-full text-center">
          ...
          <h1 className="text-2xl font-bold text-theme-dark mb-2">Honeymoon Planner</h1>
          <p className="text-warm-400 text-sm mb-8">{t('auth.syncDesc' as TranslationKey)}</p>
          <button
            onClick={login}
  ...
  // Authenticated but trips still loading from server
  if (!tripsLoaded) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-warm-50">
        <div className="flex flex-col items-center gap-4">
  ...
  // Authenticated + trips loaded but no trips — show trip management page
  if (!hasTrips) {
    return (
        ...
        <div className="min-h-screen bg-warm-50">
          <header className="sticky top-0 z-30 bg-header-bg backdrop-blur-2xl border-b border-card-border px-3 sm:px-5 py-2.5 sm:py-3 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-semibold text-theme-dark text-sm">Honeymoon Planner</span>
            </div>
            <UserMenu user={useTripStore.getState().user!} onLogout={logout} />
          </header>
          <TripListPage />
        </div>
      </Suspense>
  ...
  // Normal app — authenticated with trips
